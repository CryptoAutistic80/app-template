...
import { MetaState, Mode } from "@/src/game/types";
import { TUNING, getPalette } from "@/src/game/tuning";
import {
  loadMeta,
  saveMeta,
  spendTicket,
...
export default function HomeScreen() {
  ...
  // Reload meta when returning from game
  useEffect(() => {
    const unsubscribe = () => {
      loadMeta().then(setMeta);
    };
    // This is a simplified approach - in production you'd use focus listeners
    return unsubscribe;
  }, []);
  ...
  const handleStartRun = useCallback(
    async (mode: Mode) => {
      ...
      // Save meta with spent ticket
      await saveMeta(meta);

      // Navigate to game with mode
      router.push(`/game?mode=${mode}` as any);
    },
    [meta, router]
  );
    ...
    <LinearGradient colors={palette.bg as [string, string]} style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.titleContainer}>
          <Text style={[styles.title, { color: palette.neonA, textShadowColor: palette.neonA }]}>
            MINUTE
          </Text>
          <Text style={[styles.title, { color: palette.neonB, textShadowColor: palette.neonB }]}>
            HEIST
          </Text>
        </View>
        ...
        <View style={styles.missionsContainer}>
          ...
          {meta.missions.daily.length === 0 ? (
            <Text style={styles.noMissions}>Complete a run to see missions</Text>
          ) : (
